...
population = int(sys.argv[1]) # human population
p_covid = float(sys.argv[2]) # probability that person has covid
r_catch = float(sys.argv[3]) # probability that individual catches covid off nearby sick individual
d = float(sys.argv[4]) #distance threshold for being able to catch covid

# mask wearing and vaccine uptake
mask_wearing = int(sys.argv[5])
vaccinations = int(sys.argv[6])
...
class human:
    def __init__(self, i):
        ...
        self.job = 'student' if i < (population/3) else 'worker' if i < ((population/3)*(2)) else 'unemployed'
